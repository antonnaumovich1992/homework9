...
document.forms.saveForm.addEventListener("submit", event => {
...
function isResizeMode(x, y) {
  ...
  return (
    cropData.y - CONTROL_SIZE <= y &&
    y <= cropData.y + CONTROL_SIZE &&
    ((leftSide - CONTROL_SIZE <= x && x <= leftSide + CONTROL_SIZE) ||
      (rightSide - CONTROL_SIZE <= x && x <= rightSide + CONTROL_SIZE))
  );
}
